...
t_vec		diffusion_light(t_inter inter, t_scene *scene)
{
	...
	while (light_src)
	{
		...
		color[1] = vec_num(color_to_vec(light.color), light.intens);
		cosinus = scal_prod(l_ray[1], inter.norm) / l_ray[1].len;
		if (cosinus > 0 && !is_shadowed(l_ray, scene))
			color[0] = vec_add(color[0], vec_num(color[1], cosinus));
		light_src = light_src->next;
	}
	color[1] = vec_num(color_to_vec(scene->amb_color), scene->amb_intens);
	color[0] = vec_add(color[0], color[1]);
	color[0] = vec_num(color[0], 1. / 255);
	return (color[0]);
}
